...
class Environment:
    ...
    def step(self, action):
        ...
        if new_x < 0 or new_x >= self.board_size or new_y < 0 or new_y >= self.board_size:
            return -20, len(self.snake), True  # High negative reward for hitting wall
        
        # Check for self collision
        if new_head in self.snake:
            return -20, len(self.snake), True  # High negative reward for hitting self
        ...
        if ate_green_apple:
            ...
            self._place_green_apple()
            reward = 10  # Positive reward for eating green apple
        elif ate_red_apple:
            ...
            reward = -5  # Negative reward for eating red apple
        else:
            # Remove the tail
            tail = self.snake.pop()
            if tail in self.snake:  # If there are duplicate coordinates, don't clear the board
